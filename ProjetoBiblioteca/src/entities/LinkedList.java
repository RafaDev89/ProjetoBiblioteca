...
public class LinkedList {
    ...
    // Método para exportar a lista de livros para a árvore binária
    public void exportarParaArvore(ArvoreDeLivros arvore) {
        Noh current = head;
        while (current != null) {
            // Criar uma lista de strings com as informações dos livros
            ArrayList<String> livros = new ArrayList<>();
            livros.add(current.data.getTitulo());
            // Inserir a lista de livros na árvore
            arvore.inserir(livros);
            current = current.proximo;
        }
    }
